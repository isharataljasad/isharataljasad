...
function bindQuestions(){
  for(const el of document.querySelectorAll('[data-question]')){
    ...
    el.querySelector('form').addEventListener('submit',event=>{
      ...
      feedback.textContent=result.correct?`${name==='diagnostic'?'Good. You can move straight to the independent check.':'Correct. Explain the decision in your own words.'} ${q.reason}`:`${result.kind} difficulty: ${result.text} Review the idea, then try the fresh problem below.`;
    });
