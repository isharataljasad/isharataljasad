...
const lessons = [...document.querySelectorAll('.math-lesson')];
const keyFor = lesson => `math101:${stream}:${lesson.id}`;
...
if (search) search.addEventListener('input', () => {
  ...
  for (const unit of document.querySelectorAll('.unit:not(.index-unit):not(.appendix)')) unit.hidden = ![...unit.querySelectorAll('.math-lesson')].some(lesson => !lesson.hidden);
