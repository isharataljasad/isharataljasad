...
const lessons = [...document.querySelectorAll('.chemistry-lesson')];
const keyFor = lesson => `chemistry101:${stream}:${lesson.id}`;
...
if (search) search.addEventListener('input', () => {
  ...
  for (const unit of document.querySelectorAll('.unit')) if (unit.querySelector('.chemistry-lesson')) unit.hidden = ![...unit.querySelectorAll('.chemistry-lesson')].some(lesson => !lesson.hidden);
