...
تتوفر المتطلبات عبر بيئة بايثون المخصصة للبناء.
...
for c in manifest['courses']:
    ...
    for i,t in enumerate(c['topics']):
        ...
        body=f'''<a class="back" href="/semester-1/{c['path']}/">← {H(c['title'])}</a><section class="topic-heading"><p class="eyebrow">{c['code']} · TOPIC {t['sequence']:02d}</p><h1>{H(t['title'])}</h1><p class="lead">{H(t['outcome'])}</p><nav class="steps" aria-label="Lesson steps"><a href="#learn">Learn</a><a href="#try">Check</a><a href="#transfer">Apply</a></nav></section><section id="learn" class="explanation"><span class="section-label">THE IDEA</span><h2>Understand the decision</h2><p>{H(t['idea'])}</p><div class="worked"><h3>Worked example</h3><p>{H(t['example'])}</p></div><p class="engineering"><strong>Engineering connection</strong> {H(t['context'])}</p><a class="foundation-link" href="/semester-1/foundations/#{t['foundation']}">Need a refresher? {H(FOUNDATIONS[t['foundation']][0])} →</a></section><section class="more-help"><h2>Need more help with this topic?</h2><p>Open one explanation that helps. You do not need to complete every source.</p>{source_material(c,t)}</section><div id="try">{question('diagnostic',t['diagnostic'])}</div><div id="transfer">{question('transfer',t['transfer'])}</div><div class="finish"><p id="topic-status" role="status">Complete the new example to record your practice.</p>{nav}</div>'''
        write(BASE/c['path']/t['id']/'index.html',page(t['title'],body,c['id'],f'data-topic="{t["key"]}"'))

# Preserve the shared-library redirect when rebuilding the old semester shell.
write(BASE/'foundations'/'index.html',(ROOT/'tools/templates/foundations-redirect.html').read_text(encoding='utf-8'))
